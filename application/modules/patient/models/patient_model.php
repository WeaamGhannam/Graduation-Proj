...
class Patient_model extends CI_model {
    ...
    function getLimit() {
        $current = $this->db->get_where('patient', array('hospital_id' => "416"))->num_rows();
        $limit = 10000;
    ...
    function getPatientBySearch($search) {
        ...
        $query = $this->db->select('*')
                ...
                ->where("(name LIKE '%" . $search . "%' OR phone LIKE '%" . $search . "%' OR address LIKE '%" . $search . "%' OR email LIKE '%" . $search . "%')", NULL, FALSE)
                ->get();
