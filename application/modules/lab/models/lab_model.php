...
class Lab_model extends CI_model {
    ...
    function insertLab($data) {
        $data1 = array('hospital_id' => "416");
        $data2 = array_merge($data, $data1);
        $this->db->insert('lab', $data2);
    }

    function getLab() {
        $this->db->where('hospital_id', "416");
    ...
    function getLabByLimitBySearch($limit, $start, $search) {
        ...
        $query = $this->db->select('*')
                ->from('lab')
                ->where('hospital_id', "416")
                ->where("(id LIKE '%" . $search . "%' OR patient_name LIKE '%" . $search . "%' OR patient_id LIKE '%" . $search . "%' OR patient_phone LIKE '%" . $search . "%' OR patient_address LIKE '%" . $search . "%'OR doctor_name LIKE '%" . $search . "%'OR date_string LIKE '%" . $search . "%')", NULL, FALSE)
                ->get();
